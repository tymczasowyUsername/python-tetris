...
#----------------------------------------------------------------------------------
''' Game board width, number of blocks in single line, number of blocks in single column,
    block size can be adjusted. '''

gameboard_W = 500                                   # <300; 1500>  
blocks_in_line = 10                                 # <4; gameboard_W / 3> 
block_size = gameboard_W / blocks_in_line           # default = 25
blocks_in_column = 15                               # <4; 1000 / (gameboard_W / blocks_in_line)>

screen_W = int (gameboard_W + 0.5 * gameboard_W)    # gameboard - 500, scoreboard - 250
screen_H = int (block_size * blocks_in_column)
x = int (blocks_in_line / 2 - 1)                    # blocks spawn coordinates, see class Items
                                                     
assert 300 <= gameboard_W <= 1500, '300 <= {} <= 1500 - statement must be true'.format (gameboard_W)

assert 4 <= blocks_in_line <= gameboard_W / 3, '4 <= {} <= {} - statement\
                                                must be true'.format (blocks_in_line, gameboard_W / 3)

assert 4 <= blocks_in_column <= 1000 / block_size, '4 <= {} <= {}  - statement\
                                             must be true'.format (blocks_in_line, 1000 / block_size)
#----------------------------------------------------------------------------------
pygame.init()
Display = pygame.display.set_mode ((screen_W, screen_H))
Time = pygame.time.Clock()
Event = pygame.event
font = pygame.font.Font ('freesansbold.ttf', int (min (gameboard_W / 20, screen_H / 25)))
...
class Board:
    ...
    def move_item (self, direc):
        ...
        if direc == 'none':
            ...
            if not any (self.test_if_possible (*inc)):
                ...
                if not any (self.test_if_possible (0, 0)):
                    self.view.display_item ([], self.item.blocks, self.item.colour)

                elif not any (self.test_if_possible (0, 0, [(x - 1, 0), (x, 0), (x + 1, 0)])) :
                    self.view.display_item ([], Items.shapes[2]['blocks'], Items.shapes[2]['colour'])
                    self.stop()
    ...
    def stop (self):
        ''' Stops repeatedly calling move_item ('down')'''

        pygame.time.set_timer (EVENT_MOVEDOWN, 0)


    def save_to_array (self):
        '''Saves item's int to array'''
    ...
    def update_board (self, lines):
        '''Update array after getting full line(s)'''
...
class Board_View:
    ...
    def view_scoreboard (self, level, score, colour):
        ...
        self.display_block (gameboard_W, 0, white, gameboard_W * 0.5, screen_H)

        text1 = font.render (('Level ' + str (level)), True, colour)
        rect1 = text1.get_rect (center = (gameboard_W * (10 / 8), screen_H * (1 / 13)))
        Display.blit (text1, rect1)

        text2 = font.render (('Score: ' + str (score)), True, colour)
        rect2 = text2.get_rect (center = (gameboard_W * (10 / 8), screen_H * (2 / 13)))
        Display.blit (text2, rect2)
        

class Myarray (np.ndarray):
    '''Sublcass of numpy array that prevents items to exceed game bounds.'''
